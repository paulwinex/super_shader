...
> Если такого файла нет то беретсяфайл поумолчанию `_super_map_example.json`. Для создания своей карты просто скопируйте файл `_super_map_example.json` и переименуйте в `_super_map.json`. В дальнейшем обновление сприта не затрёт вашу карту.
